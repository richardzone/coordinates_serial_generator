...
class RandomCoordinatesCommand(Command):
    ...
    def generate_random_coordinates(self) -> Tuple[int, int]:
        return random.randint(0, self.screen_width), random.randint(
...
def main():
    ...
    parser.add_argument(
        "--rate", type=int, default=9600, help="Baud rate (default: 9600)"
    )
    parser.add_argument(
        "--min-interval",
        type=int,
        default=500,
        help="Min interval in milliseconds between commands (default: 500)",
    )
    parser.add_argument(
        "--max-interval",
        type=int,
        default=4000,
        help="Max interval in milliseconds between commands (default: 4000)",
    )
